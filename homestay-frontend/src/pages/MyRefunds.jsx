...
const statusStyle = {
  PENDING: { bg: '#FEF3C7', color: '#92400E', label: 'Chờ Host chuyển tiền' },
  SENT: { bg: '#DBEAFE', color: '#1E40AF', label: 'Host báo đã chuyển' },
  CONFIRMED: { bg: '#DCFCE7', color: '#166534', label: 'Hoàn tất' }
};
...
const MyRefunds = () => {
  ...
  const executeConfirmAction = async () => {
    ...
    try {
      setBusyId(id);
      await refundService.confirmReceived(id);
  ...
  return (
    <div className="container page">
      <div className="page-header">
        <h1 style={{ margin: 0 }}>Hoàn tiền của tôi</h1>
      </div>

      <div className="filter-bar">
        {['ALL', 'PENDING', 'SENT', 'CONFIRMED'].map((s) => (
          <button
            key={s}
            type="button"
            onClick={() => setFilter(s)}
          ...
          >
            {s === 'ALL' ? 'Tất cả' : s === 'PENDING' ? 'Chờ hoàn tiền' : s === 'SENT' ? 'Cần xác nhận' : 'Hoàn tất'}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="empty-state">
          <p style={{ margin: 0 }}>Không có dữ liệu hoàn tiền.</p>
        </div>
      ) : (
        <>
                ...
                <div key={r.id} className="list-card is-column">
                  <div className="list-card-title-row">
                    <div>
                      ...
                      <div className="list-card-meta">
                        Lý do: {r.reason === 'REJECTED' ? 'Host từ chối đơn' : 'Bạn hủy đơn'}
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '24px 48px', fontSize: '0.9rem', marginBottom: '12px' }}>
                    <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start', minWidth: '240px' }}>
                      ...
                      <div>
                        <div style={{ fontWeight: 600, color: '#059669', fontSize: '1rem' }}>
                          Số tiền được hoàn: {Number(r.amount || 0).toLocaleString('vi-VN')} ₫
                        </div>
                    ...
                    <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start', minWidth: '200px' }}>
                      <Building2 size={16} style={{ marginTop: 2, flexShrink: 0 }} />
                      <div>
                        <div><strong>Homestay</strong></div>
                        <div className="list-card-meta">{r.homestayTitle || '—'}</div>
                      </div>
                    </div>
                  </div>
      ...
      {confirmModal.show && (
        <div style={{
          ...
          <div style={{ background: '#fff', padding: 24, borderRadius: 12, width: 400, maxWidth: '90%' }}>
            <h3 style={{ margin: '0 0 16px 0' }}>Xác nhận nhận tiền</h3>
            <p style={{ lineHeight: 1.6, marginBottom: 24 }}>
              Bạn chắc chắn đã nhận được số tiền hoàn này trong tài khoản ngân hàng của mình?
            </p>
